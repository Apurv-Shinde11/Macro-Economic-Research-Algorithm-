...
def _run_pipeline_sync(job_id: str, user_id: str, repo: float, deficit: float, capex: float):
    try:
        ...
        try:
            _supabase.table("run_history").insert({"user_id": user_id, "regime": regime.get("regime", ""), "confidence": regime.get("confidence", 0), "conviction": strat.get("conviction", ""), "repo_rate": repo, "deficit": deficit, "capex": capex, "summary": dec.get("summary", ""), "report_text": report if isinstance(report, str) else "", "allocation": pos.get("allocation", {}), "stress_test": {"repo_rate": repo, "deficit": deficit, "capex": capex}}).execute()
...
async def get_history(limit: int = 30, profile: dict = Depends(require_access)):
    result = _supabase.table("run_history").select("*").eq("user_id", profile["id"]).order("run_at", desc=True).limit(limit).execute()
